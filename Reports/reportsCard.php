...
if (isset($_GET['key_rpt_card'])) 
{
	if ($_GET['key_rpt_card'] == 'all') 
	{
			// Recebe o retorno do método relatorioGeralCard
		$listaCards = $controllerCard->relatorioGeralCard();

				 // Total de registros
		$total_registros = $listaCards->count();
		...
		foreach ($listaCards as $obj) {
            ...
            //Adicionando Css nos relatórios
				...
				$mpdf->SetTitle("relatorio-geral-cards-cadastrados");

				$mpdf->WriteHTML($html);

				$mpdf->Output("relatorio-geral-cards.pdf", "I");
	...
	else if ($_GET['key_rpt_card'] == 'especific') 
	{
		$pagina_atual = 1;
		...
		{
			$id_card= intval($_GET['id_card']);
				// Recebe o retorno do método relatorioEspecificoCard
			$card = $controllerCard->relatorioEspecificoCard($id_card);

			$html = "<div class='logo'> <img src='../Imagens/curumim_logo.png'></div>
			<h1>Escola de Educação Infantil Papa-Capim Ltda.</h1>
			<div class='dadosEmissao'><p>Data: 14/05/2018</p></div>
			...
			<tbody>
			<tr>
			<td>{$card->getDescricao()}</td>
			<td>{$card->getDataCadastro()}</td>
			</tr>              
			</tbody>	
			</table>";
			...
			$mpdf->SetTitle("relatorio-especifico-card-cadastrado");

			$mpdf->WriteHTML($html);

			$mpdf->Output("relatorio-especifico-card.pdf", "I");
